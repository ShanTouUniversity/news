...
- **信息完整**：必须保证信息完整传达，关键信息（项目全称、人名、数字、时间、地点等）不可遗漏或模糊处理
- **中英文一致**：英文版必须表达和中文版**相同的意思**，同一关键信息不可只出现在一种语言版本中
...
### 完成输出
每完成一篇文章的创建/修改后，输出中英标题及对应路径，格式如下：
```
zh: 《中文标题》 → src/content/news/zh/YYYY/MM/DD/slug.md
en: "English Title" → src/content/news/en/YYYY/MM/DD/slug.md
```
...
### 验证
为节省时间，**不在每篇文章创建后立即构建**，而是在提交前统一执行 `pnpm build` 确认整体构建无报错。

> **重要**：除非用户明确要求提交或构建，否则不要在创建文章后自动运行 `pnpm build`。
